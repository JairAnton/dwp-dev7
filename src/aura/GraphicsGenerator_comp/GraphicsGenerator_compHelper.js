            ...
            if (state === "SUCCESS") {
                ...
                if (reportResultData != null) {
                    ...
                    for (var i = 0; i < reportResultData.length; i++) {
						if(i>0 && reportResultData[i].length>6 && device!=='DESKTOP') {
							startMonth=6;
						}
                        if (reportResultData.length > 2) {
                            for (var j = 0; j < reportResultData[i].length-startMonth; j++) {
                                switch (i) {
                                    case 0:
                                        concepto[j] = reportResultData[i][j];
                                        break;
                                    case 1:
                                        nombreColumnas[j] = reportResultData[i][j+startMonth];
                                        break;
                                    case 2:
                                        chartData[j] = reportResultData[i][j+startMonth];
                                        numeroGraficaCentro = numeroGraficaCentro + parseInt(chartData[j]);
                                        break;
                                    case 3:
                                        chartData2[j] = reportResultData[i][j+startMonth];
                                        break;
                                    case 4:
                                        chartData3[j] = reportResultData[i][j+startMonth];
                                        break;
                                    case 5:
                                        chartData4[j] = reportResultData[i][j+startMonth];
                                        break;
                                    case 6:
                                        chartData5[j] = reportResultData[i][j+startMonth];
                                        break;
                                }
                            }
                        }
