...
export const setClientSection = (clientSections, marginSum, commSum) => {
  let marginIndex = clientSections.findIndex(
    (obj) => obj.row === "Margen Financiero"
  );
  if (marginIndex >= 0) {
    clientSections[marginIndex].cyAP = marginSum;
    clientSections[marginIndex].cyEstim =
      marginSum + clientSections[marginIndex].currentYear;

    clientSections[marginIndex].iconNameCY = `utility:${
      clientSections[marginIndex].priorYear >
      clientSections[marginIndex].cyEstim
        ? "arrowdown"
        : "arrowup"
    }`;
  ...
  let commissionIndex = clientSections.findIndex(
    (obj) => obj.row === "Comisiones"
  );
  if (commissionIndex >= 0) {
    clientSections[commissionIndex].cyAP = commSum;
    clientSections[commissionIndex].cyEstim =
      commSum + clientSections[commissionIndex].currentYear;
    clientSections[commissionIndex].iconNameCY = `utility:${
      clientSections[commissionIndex].priorYear >
      clientSections[commissionIndex].cyEstim
        ? "arrowdown"
        : "arrowup"
    }`;
  ...
  let mrgOrdnIndex = clientSections.findIndex(
    (obj) => obj.row === "Margen Ordinario"
  );
  if (mrgOrdnIndex >= 0) {
    clientSections[mrgOrdnIndex].cyAP =
      clientSections[marginIndex].cyAP + clientSections[commissionIndex].cyAP;
    clientSections[mrgOrdnIndex].cyEstim =
      clientSections[marginIndex].cyEstim +
      clientSections[commissionIndex].cyEstim;

    clientSections[mrgOrdnIndex].iconNameCY = `utility:${
      clientSections[mrgOrdnIndex].priorYear >
      clientSections[mrgOrdnIndex].cyEstim
        ? "arrowdown"
        : "arrowup"
    }`;
...
export const calculator = (
  ...
  let months =
    current.term <= 12 - (expectedDate.getMonth() + 1)
      ? current.term
      : 12 - (expectedDate.getMonth() + 1);
  ...
  if (current.opportunityName === RIESGO_DE_FIRMA) {
    ...
    current.calculatedMargin = Number(
      (current.opportunityValue * (months * current.calcSpreadM)).toFixed(2)
    );
  }

  current.calComm = Number(
    ((current.opportunityValue * current.strComm) / 100).toFixed(2)
  );
  ...
  let cRentabilityIndex = rentabilityDataMap.data.findIndex(
 ...
 *
 */
export const updateEstimationList = (
  estimationList,
  currentEstimationIndex,
  eventData
) => {
  ...
  if (eventData[0].hasOwnProperty("spread")) {
    ...
    currEstimation.calcSpreadM =
      Math.pow(1 + currEstimation.spread / 100, 1 / 12) - 1;
  }
